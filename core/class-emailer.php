<?php
/**
 * Emailer class.
 *
 * Usage Examples:
 * ==============
 * // See what's scheduled
 * $pending = $emailer->get_pending_emails();

 * // Cancel specific email
 * $emailer->cancel_email($email_id);

 * // Cancel entire sequence
 * $emailer->cancel_sequence($sequence_id);
 *
 * // Create a follow up sequence
 * $sequence_id = $emailer->create_followup_sequence(
 *   $customer_email,
 *   array(
 *     array('days' => 1, 'subject' => 'Thank you!', 'message' => '...'),
 *     array('days' => 3, 'subject' => 'How is it?', 'message' => '...'),
 *     array('days' => 7, 'subject' => 'Review?', 'message' => '...'),
 *     array('days' => 14, 'subject' => 'Special offer', 'message' => '...')
 *   ),
 *   array('customer_name' => $customer_name, 'order_id' => $order_id)
 * );
 *
 * // Thank you email in 1 day
 * $emailer->send_later(
 *   $customer_email,
 *   'Thanks for your purchase!',
 *   '<h1>Thank you {customer_name}!</h1>',
 *   1,
 *   array('customer_name' => $customer_name)
 * );
 *
 * @package store-boost-kit\admin\
 * @author Store Boost Kit <hello@storeboostkit.com>
 * @version 1.0
 */

namespace STOBOKIT;

defined( 'ABSPATH' ) || exit;

class Emailer {

	private static $instance = null;
	private $template_dir;
	private $defaults = array(
		'from_name' => '',
		'from_email' => '',
		'content_type' => 'text/html',
		'charset' => 'UTF-8',
	);

	private $shortcodes = array();

	private function __construct() {
		$this->init();
	}

	public static function get_instance() {
		if ( null === self::$instance ) {
			self::$instance = new self();
		}
		return self::$instance;
	}
    
    private function init() {
        $this->template_dir = WP_CONTENT_DIR . '/email-templates/';
        $this->defaults['from_email'] = get_option('admin_email', '');
        $this->defaults['from_name'] = get_option('blogname', '');
        
        $this->register_default_shortcodes();
        
        // Hook for individual email sending
        add_action('stobokit_emailer_send_single', array($this, 'send_scheduled_email'), 10, 1);
    }
    
    /**
     * Send email immediately
     */
    public function send_now($to, $subject, $message, $args = array()) {
        $args = wp_parse_args($args, $this->defaults);
        
        $subject = $this->process_shortcodes($subject, $args);
        $message = $this->process_shortcodes($message, $args);
        $headers = $this->build_headers($args);
        $attachments = isset($args['attachments']) ? $args['attachments'] : array();
        
        return wp_mail($to, $subject, $message, $headers, $attachments);
    }
    
    /**
     * Schedule single email with individual cron job
     */
    public function send_later($to, $subject, $message, $days_later, $args = array()) {
        $send_time = current_time('timestamp') + ($days_later * DAY_IN_SECONDS);
        $email_id = 'email_' . uniqid();
        
        // Create email data
        $email_data = array(
            'to' => $to,
            'subject' => $subject,
            'message' => $message,
            'args' => $args,
            'email_id' => $email_id
        );
        
        // Store email data
        update_option('stobokit_emailer_data_' . $email_id, $email_data);
        
        // Schedule individual cron job for this specific email
        wp_schedule_single_event($send_time, 'stobokit_emailer_send_single', array($email_id));
        
        return $email_id;
    }
    
    /**
     * Create follow-up sequence with individual cron jobs
     */
    public function create_followup_sequence($to, $sequence, $args = array()) {
        $sequence_id = 'seq_' . uniqid();
        $email_ids = array();
        $base_time = current_time('timestamp');
        
        foreach ($sequence as $index => $email) {
            $send_time = $base_time + ($email['days'] * DAY_IN_SECONDS);
            $email_id = $sequence_id . '_' . $index;
            
            // Create email data
            $email_data = array(
                'to' => $to,
                'subject' => $email['subject'],
                'message' => $email['message'],
                'args' => $args,
                'email_id' => $email_id,
                'sequence_id' => $sequence_id
            );
            
            // Store email data
            update_option('stobokit_emailer_data_' . $email_id, $email_data);
            
            // Schedule individual cron job
            wp_schedule_single_event($send_time, 'stobokit_emailer_send_single', array($email_id));
            
            $email_ids[] = $email_id;
        }
        
        // Store sequence info
        update_option('stobokit_emailer_sequence_' . $sequence_id, $email_ids);
        
        return $sequence_id;
    }
    
    /**
     * Send scheduled email (called by individual cron job)
     */
    public function send_scheduled_email($email_id) {
        // Get email data
        $email_data = get_option('stobokit_emailer_data_' . $email_id);
        
        if (!$email_data) {
            return false;
        }
        
        // Send the email
        $sent = $this->send_now(
            $email_data['to'],
            $email_data['subject'],
            $email_data['message'],
            $email_data['args']
        );
        
        // Clean up - delete the email data
        delete_option('stobokit_emailer_data_' . $email_id);
        
        // Log the send
        $this->log_email($email_data, $sent);
        
        return $sent;
    }
    
    /**
     * Cancel scheduled email
     */
    public function cancel_email($email_id) {
        // Remove the cron job
        wp_clear_scheduled_hook('stobokit_emailer_send_single', array($email_id));
        
        // Remove email data
        delete_option('stobokit_emailer_data_' . $email_id);
        
        return true;
    }
    
    /**
     * Cancel entire sequence
     */
    public function cancel_sequence($sequence_id) {
        $email_ids = get_option('stobokit_emailer_sequence_' . $sequence_id, array());
        
        foreach ($email_ids as $email_id) {
            $this->cancel_email($email_id);
        }
        
        // Remove sequence info
        delete_option('stobokit_emailer_sequence_' . $sequence_id);
        
        return true;
    }
    
    /**
     * Get purchase follow-up sequence
     */
    public function get_purchase_sequence() {
        return array(
            array(
                'days' => 1,
                'subject' => 'Thanks for your purchase from {site_name}!',
                'message' => '<h1>Thank you {customer_name}!</h1><p>Your order #{order_id} for {product_name} is confirmed. Total: {order_total}</p><p>We appreciate your business!</p>'
            ),
            array(
                'days' => 3,
                'subject' => 'How is your {product_name}?',
                'message' => '<h1>Quick check-in</h1><p>Hi {customer_name}, how are you enjoying your {product_name}? Any questions or feedback?</p>'
            ),
            array(
                'days' => 7,
                'subject' => 'Would you mind leaving a review?',
                'message' => '<h1>Share your experience</h1><p>Hi {customer_name}, if you\'re happy with your {product_name}, we\'d love a review!</p><p><a href="{site_url}/reviews" style="background: #007cba; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px;">Leave Review</a></p>'
            ),
            array(
                'days' => 14,
                'subject' => 'Exclusive offer for valued customers',
                'message' => '<h1>Special Discount</h1><p>Hi {customer_name}, enjoy 15% off your next purchase as our thank you!</p><p>Use code: THANKYOU15</p>'
            )
        );
    }
    
    /**
     * Quick method for purchase follow-ups
     */
    public function start_purchase_followups($customer_email, $customer_name, $order_id, $product_name, $order_total) {
        return $this->create_followup_sequence(
            $customer_email,
            $this->get_purchase_sequence(),
            array(
                'customer_name' => $customer_name,
                'order_id' => $order_id,
                'product_name' => $product_name,
                'order_total' => $order_total
            )
        );
    }
    
    /**
     * Get all pending emails info
     */
    public function get_pending_emails() {
        global $wpdb;
        
        // Get all scheduled single events for our action
        $crons = _get_cron_array();
        $pending = array();
        
        foreach ($crons as $timestamp => $cron) {
            if (isset($cron['stobokit_emailer_send_single'])) {
                foreach ($cron['stobokit_emailer_send_single'] as $job) {
                    $email_id = $job['args'][0];
                    $email_data = get_option('stobokit_emailer_data_' . $email_id);
                    
                    if ($email_data) {
                        $pending[] = array(
                            'email_id' => $email_id,
                            'to' => $email_data['to'],
                            'subject' => $email_data['subject'],
                            'send_time' => date('Y-m-d H:i:s', $timestamp),
                            'days_remaining' => ceil(($timestamp - time()) / DAY_IN_SECONDS)
                        );
                    }
                }
            }
        }
        
        return $pending;
    }
    
    /**
     * Helper methods
     */
    private function build_headers($args) {
        $headers = array();
        
        if (!empty($args['content_type'])) {
            $headers[] = 'Content-Type: ' . $args['content_type'] . '; charset=' . $args['charset'];
        }
        
        if (!empty($args['from_email'])) {
            $from = $args['from_email'];
            if (!empty($args['from_name'])) {
                $from = $args['from_name'] . ' <' . $args['from_email'] . '>';
            }
            $headers[] = 'From: ' . $from;
        }
        
        return $headers;
    }
    
    private function register_default_shortcodes() {
        $this->register_shortcode('site_name', function($args) {
            return get_bloginfo('name');
        });
        
        $this->register_shortcode('site_url', function($args) {
            return get_site_url();
        });
        
        $this->register_shortcode('customer_name', function($args) {
            return isset($args['customer_name']) ? $args['customer_name'] : '';
        });
        
        $this->register_shortcode('order_id', function($args) {
            return isset($args['order_id']) ? $args['order_id'] : '';
        });
        
        $this->register_shortcode('product_name', function($args) {
            return isset($args['product_name']) ? $args['product_name'] : '';
        });
        
        $this->register_shortcode('order_total', function($args) {
            return isset($args['order_total']) ? $args['order_total'] : '';
        });
        
        $this->register_shortcode('current_date', function($args) {
            $format = isset($args['format']) ? $args['format'] : 'F j, Y';
            return date($format);
        });
    }
    
    public function register_shortcode($name, $callback) {
        $this->shortcodes[$name] = $callback;
    }
    
    private function process_shortcodes($content, $args = array()) {
        foreach ($this->shortcodes as $name => $callback) {
            $pattern = '/\{' . preg_quote($name, '/') . '(?:\:([^}]*))?\}/';
            $content = preg_replace_callback($pattern, function($matches) use ($callback, $args) {
                return call_user_func($callback, $args);
            }, $content);
        }
        return $content;
    }
    
    private function log_email($email_data, $sent) {
        $logs = get_option('stobokit_emailer_logs', array());
        $logs[] = array(
            'to' => $email_data['to'],
            'subject' => $email_data['subject'],
            'email_id' => $email_data['email_id'],
            'sent' => $sent,
            'sent_at' => current_time('mysql')
        );
        
        // Keep only last 100 logs
        if (count($logs) > 100) {
            $logs = array_slice($logs, -100);
        }
        
        update_option('stobokit_emailer_logs', $logs);
    }
}